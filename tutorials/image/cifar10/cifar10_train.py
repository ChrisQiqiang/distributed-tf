...
tf.app.flags.DEFINE_string('worker_hosts', '10.2.99.4:2222',
                    'Comma-separated list of hostname:port pairs')
tf.app.flags.DEFINE_string('job_name', None, 'job name: worker or ps')
tf.app.flags.DEFINE_integer('task_index', None, 'Index of task within the job')
tf.app.flags.DEFINE_integer('sync', None, 'Whether synchronization: 0 or 1')
...
##        summaries.append(tf.summary.histogram(var.op.name + '/gradients', grad))
    ...
    if issync == 1:
    	...
    	apply_gradient_op = syn_opt.apply_gradients(grads, global_step=global_step)
	 # Newly added
	if is_chief:
...
#    for step in xrange(FLAGS.max_steps):
      ...
      print ('worker %d: step %d, global step %d: loss = %.2f' % (FLAGS.task_index, step, g_step, loss_value))
